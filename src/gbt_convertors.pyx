                ...
                if tree_depth == 0:
                    ...
                    for cur_level in range(cur_tree_depth - 2, -1, -1):
                        ...
                        for cur_parent in prev_level_nodes:
                            ...
                            cur_left_node = mb.add_split(tree_id=tree_id, parent_id=cur_parent, position=0,
                                                         feature_index=cur_level_split['feature_index'], feature_value=cur_level_split['value'])
                            cur_right_node = mb.add_split(tree_id=tree_id, parent_id=cur_parent, position=1,
                                                          feature_index=cur_level_split['feature_index'], feature_value=cur_level_split['value'])
                    ...
                    if not is_classification or n_classes == 2:
                        for last_level_node_num in range(len(prev_level_nodes)):
                            mb.add_leaf(tree_id=tree_id, response=current_tree_leaf_val[2 * last_level_node_num]
                                        * scale + bias, parent_id=prev_level_nodes[last_level_node_num], position=0)
                            mb.add_leaf(tree_id=tree_id, response=current_tree_leaf_val[2 * last_level_node_num + 1]
                        ...
                        for last_level_node_num in range(len(prev_level_nodes)):
                            ...
                            mb.add_leaf(
                                tree_id=tree_id, response=current_tree_leaf_val[left_index] * scale + bias, parent_id=prev_level_nodes[last_level_node_num], position=0)
                            mb.add_leaf(
                                tree_id=tree_id, response=current_tree_leaf_val[right_index] * scale + bias, parent_id=prev_level_nodes[last_level_node_num], position=1)
    else:
        for class_label in range(n_tree_each_iter):
            for i in range(n_iterations):
                ...
                if root_node.value is None:
                    ...
                    while nodes_queue:
                        ...
                        if right_node.value is None:
                            ...
                            mb.add_leaf(
                                tree_id=tree_id, response=cur_node.right.value[class_label],
